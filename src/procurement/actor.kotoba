(ns procurement.actor
  "ProcurementActor — the ISCO-08 3323 community procurement actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one procurement operation request
  ...
  The unconditional invariant: the ProcurementAdvisor can never
  directly commit a record the ProcurementGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [procurement.advisor :as advisor]
            [procurement.governor :as governor]
            [procurement.store :as store]))

(defn build-graph
  "Build a compiled ProcurementActor graph. `store` implements
  `procurement.store/Store`. `advisor` implements
  `procurement.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:client-id (:client-id request)
                                    :op (:op proposal)
                                    :supplier-id (:supplier-id proposal)
                                    :payload proposal}]
